...
- **Committed-generators-hash enforcement (E1):** set `dcc.blockchain.custom.functionality.enforce-committed-generators-hash-from-height` to the **first generation-period boundary at/after feature #25 activates** (on a fresh mainnet with no legacy pre-hash blocks, this can be the first boundary). This makes the per-block `committedGeneratorsHash` — the compensating control for the state-hash committee exclusion — **mandatory** rather than optional. It is a height-gate (like `enforce-transfer-validation-after`): default `0` = off, which is required so the existing testnet (which has legacy pre-hash boundary blocks) is not broken; only ever set it to a height where every producer already emits the hash. `maxCommittedGenerators` is already on by default (cap 1000) — no action needed.
...
2. **Known-peers blacklist exemption — ✅ DONE in node-scala** (`feat/endorsement-rebroadcast`, PR #21). `PeerDatabaseImpl.blacklistOrSuspend` (the automatic protocol-trigger path used by `blacklistAndClose`) now **suspends a configured known-peer briefly instead of applying the 15m ban** — untrusted peers still get the full blacklist. The low-level `blacklist()` and the operator `/debug/blacklist` stay unconditional (deliberate admin action). Unit-tested + `BlacklistTestSuite` green. This is the targeted root-cause fix; the sentry topology (1) is still the structural defense-in-depth.
3. **Config:** re-enable `enable-blacklisting=yes`, populate a **curated `known-peers`** across regions/ASNs (seeds + sentries), keep `black-list-residence-time` short (≤15m), keep defaults (`max-single-host-connections=3`, in/out caps 100), keep `suspension-residence-time` + `invalid-blocks-storage.timeout` short so any transient ban self-heals. Optional: set `dcc.network.min-supported-app-version` to a **conservative** major (below the version deployed everywhere) to refuse downgraded peers — leave `0` unless you have a reason, since a too-high floor can partition a rolling upgrade.
...
- [ ] `enforce-committed-generators-hash-from-height` set to the first period boundary ≥ #25 activation (E1).
