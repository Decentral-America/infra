...
- **Wire matcher's `fullCheck` into CI.** Today `ci.yml`'s integration job runs `quickCheck` only (unit tests); `dex-it`/`dex-integration-it` — real E2E suites — never gate a merge. Move `fullCheck` to run in CI (see Tier 7 for cadence), since the release-artifact/live-node blocker noted in the existing `ci.yml` comment needs re-evaluation now that a release process exists.
- **New cross-repo settlement E2E spec**: `DecentralChain/packages/e2e-blockchain/src/e2e/dex-settlement-flow.spec.ts`, alongside existing `defi-flow.spec.ts` and `token-launch.spec.ts`. Flow: fund wallet, transfer, place order via matcher REST, exchange settles, verify on-chain balance and node state. Runs against local docker-compose (fast, CI-gated) and against live testnet (smoke, via `admin-e2e.yml` extended to also drive matcher, not just node — closes the finding that `admin-e2e.yml` never touches matcher today).
- **Add `CommitToGeneration` SDK-level test** to `e2e-blockchain/src/transactions/` — currently the only one of 21 node-scala transaction types with zero SDK/E2E coverage.
